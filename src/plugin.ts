...
export class RspackSplashScreenPlugin implements RspackPluginInstance {
  ...
  private transformHtml(html: string): string {
    ...
    const b = baseStyles
      .replace("/*BG_SPLASH*/", this.options.splashBg)
      .replace(/#vpss/g, `#${this.options.id}`)
      .replace(/--vpss-/g, `--${this.options.id}-`);
    const l = loaderStyles
      .replace("/*BG_LOADER*/", this.options.loaderBg)
      .replace(/#vpss/g, `#${this.options.id}`)
      .replace(/\.vpss-/g, `.${this.options.id}-`);
...
function splashTemplate({
  ...
  // Replace vpss with the custom id in loader HTML
  loaderHtml = loaderHtml.replace(/vpss-/g, `${id}-`);
